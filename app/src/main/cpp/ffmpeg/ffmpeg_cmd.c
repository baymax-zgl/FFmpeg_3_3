...
void custom_log(void *ptr, int level, const char *fmt, va_list vl) {
    FILE *fp = fopen("/storage/emulated/0/Android/data/com.onzhou.ffmpeg.cmd/files/av_log.txt",
                     "a+");
    if (fp) {
        vfprintf(fp, fmt, vl);
        fflush(fp);
...
JNIEXPORT jint JNICALL Java_com_hudun_ffmpeg3_13_19_FFmpegCmd_exec
